...
for col,new in [("volume_distance_vector_alaw","vol_alaw"),
                (VOL_BIN,"vol_bin"),
                ("port_distance_vector_alaw","port_alaw"),
                (PORT_BIN,"port_bin")]:
    df[new] = df[col].apply(parse_vec)

df["features"] = df.apply(lambda r:
        np.concatenate([r.vol_alaw, r.vol_bin, r.port_alaw, r.port_bin]), axis=1)
...
print("🌳 Entrenando RF con puertos…")
...
plt.title('RF con puertos – Matriz de confusión')
plt.xlabel('Predicción'); plt.ylabel('Real')
plt.tight_layout(); plt.savefig('confusion_matrix_rf_conpuertos.png', dpi=300); plt.close()
...
plt.xlabel('FPR'); plt.ylabel('TPR'); plt.title('RF con puertos – ROC')
plt.legend(); plt.grid(alpha=0.3); plt.tight_layout()
plt.savefig('roc_curve_rf_conpuertos.png', dpi=300); plt.close()

print("\n✅ Imágenes guardadas: confusion_matrix_rf_conpuertos.png / roc_curve_rf_conpuertos.png")
